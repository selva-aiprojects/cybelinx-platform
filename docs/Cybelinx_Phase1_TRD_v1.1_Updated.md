...
## 37.1 Application
  ...
  Backend             NestJS + Node.js + TypeScript             Recommended
  API                 REST + JSON                               Yes
  API Documentation   OpenAPI / Swagger                         Yes
  ORM                 Prisma                                    Recommended
  Validation          class-validator / Zod where appropriate   Yes
  Testing             Jest + Supertest + Playwright             Recommended
...
## 37.2 IAM
  ...
  Enterprise SSO   SAML/OIDC where provider supports it
  MFA              Existing IdP
  Authorization    Cybelinx RBAC
  Token            JWT/OIDC token

**Final IdP selection is pending existing-product assessment.**
...
## 37.3 Data
  ...
  ORM                Prisma
  Migrations         Prisma migrations + SQL for DB security objects
  Reporting Store    PostgreSQL initially

PostgreSQL 16/17/18 are acceptable; standardize on the version already
supported by the deployment environment rather than upgrading purely for
the platform.
...
## 37.4 Messaging
  ...
  ---------------- -----------------------------
  Outbox           PostgreSQL
  Worker           NestJS / Node.js
  Queue            PostgreSQL-backed initially
  Optional Queue   Redis + BullMQ
  Retry            Worker
  DLQ              PostgreSQL/Redis-backed
  Future           Managed broker / Kafka
...
## 37.5 Reporting
  ...
  Aggregations   SQL/materialized views
  Dashboard      Metabase
...
## 37.6 Platform
  ...
  Monitoring         Existing platform monitoring
...
# 67. Final Technical Stack Summary
  ...
  Backend                 NestJS / Node.js /      Modular services if
                          TypeScript              required

  API                     REST / JSON             REST + events

  IAM                     Existing IdP + Cybelinx Enterprise federation
                          RBAC                    

  Database                PostgreSQL              Distributed/data
                                                  platform if justified

  ORM                     Prisma                  Same

  Tenant Isolation        Pool / Schema /         Dedicated infrastructure
                          Dedicated DB            

  Provisioning            NestJS Worker           Workflow/orchestration
                                                  platform

  Outbox                  PostgreSQL              Same pattern

  Messaging               PostgreSQL-backed /     Managed broker / Kafka
  ...
  Audit                   PostgreSQL              Immutable/central audit
                                                  platform
  ...
  Monitoring              Existing platform       Central observability
                                                  platform

  Data Platform           Not required            Snowflake/Databricks
                                                  when justified
